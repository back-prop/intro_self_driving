...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
	...
	// your code here
	// All cells are initialized with equal probability given by 1./(height*width)
...
vector< vector <float> > move(int dy, int dx, vector < vector <float> > beliefs, float blurring) {
	...
	for (int i=0;i<height;i++) {
		for (int j=0;j<width;j++) {
			// Same as in the case of blur, new index is wrapped around 
			// the grid using the mod operator. As the vehicle moves, the
			// the new beliefs are updated by shifting them dx cells to the right 
			// and dy cells down.
	...
	float p_miss) 
{
	...
	for (int i=0;i<height;i++){
		vector <float> row;
		for (int j=0;j<width;j++){
			// We update our belief after measurement by multiplying 
			// the measurement probablility with the current belief. 
			// This now gives us the new belief
			int hit = (color == grid[i][j]);
			row.push_back(p_hit*beliefs[i][j]*hit +(1-hit)*p_miss*beliefs[i][j]);
        }
		newGrid.push_back(row);
    }
	float sum = 0.0;
	...
	// To convert the new beliefs into a probability distribution
	// we normalize the new belief matrix.
	for (int i=0;i<height;i++){
		for (int j=0;j<width;j++){

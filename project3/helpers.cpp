...
vector< vector<float> > normalize(vector< vector <float> > grid) {
	...
	// For continuous probability distribution, the height of the 
	// probablility density function can be greater than 1. 
	// This normalize(grid) function can be used to enforce the 
	// condition of probabilities adding up to 1. 
	...
	for (int i=0;i<grid.size();i++) {
		...
		for ( int j=0;j<grid[0].size();j++) {
			// normalizing every element by dividing it by total.
            ...
            for (int dx=-1;dx<2;dx++) {
                for (int dy=-1;dy<2;dy++) {
                    float mult = window[dx+1][dy+1];
                  	// As explained in the introduction for the project, in C++
                  	// -1 % 3 will return -1 . But since we are wraping around the grid
                  	// we expect the answer to be 2. To get that we need to add height &
                  	// width to the index as shown below.
                    int new_i = (i + dy + height) % height;
                    int new_j = (j + dx + width) % width;
                  	//cout << "i,j " << i << "," << j << " dx,dy " << dx << "," << dy <<" new_i,new_j " << new_i << "," << new_j << endl;

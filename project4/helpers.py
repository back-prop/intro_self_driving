...
def load_california():
    map_nodes = pd.read_csv('data/cal.cnode1',header=None,sep=' ')
    map_edges = pd.read_csv('data/cal.cedge1',header=None,sep=' ')
    map_cali = {}
    for i,node in enumerate(map_nodes.iterrows()):
        #print()
        map_cali[node[0]] = {'pos': (node[1][1],node[1][2]),
                             'connections':map_edges.loc[node[0]][:3].astype(int).tolist()}
    G = load_map_graph(map_cali)
    return Map(G)
...
def show_map(M, start=None, goal=None, path=None):
    ...
    fig = Figure(data=Data([edge_trace, node_trace]),
                 layout=Layout(
                    ...
                    xaxis=XAxis(showgrid=False, zeroline=False, showticklabels=False,range=[-125,-114]),
                    yaxis=YAxis(showgrid=False, zeroline=False, showticklabels=False,range=[32,43])))
